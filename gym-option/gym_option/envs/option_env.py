...
class OptionEnv(gym.Env):
  ...
  def __init__(self):
    ...
    self.terminal_state = 56#self.encode(self.goal) #56
    ...
    room = [r for r, offset in enumerate(self.offsets[1:5]) if self.start_state < offset][0]
    self._reset()
  ...
  def step(self, action, option):
    ...
    if self.state == self.terminal_state and option == False:
...
#rajouter une condition qui dit que si on est dans le pre-hallway mais qu'on prend pas la porte on agit normalement
    elif tuple(coord) in self.pre_hallways[room].keys():
           #Here we get [a,b] a = direction to take to go in hallway, b = kind of room but it depends on the hallway
           ...
           if action == hallway_info[0]:
             ...
             #If we are on a pre_halway and we don't go to the hallway
             [row, col] = coord
...
#    self.state = self.start_state if not isinstance(self.start_state, str) else np.random.randint(self.n_states - 1)
    start_room = 0
    sz = self.room_sizes[start_room]
    self.state = self.offsets[start_room] + np.random.randint(sz[0]*sz[1] - 1)
    self.done = False
    return self.state
